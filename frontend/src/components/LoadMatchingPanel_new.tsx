                ...
                <div className="flex-1">
                  ...
                  <div className="space-y-1 text-sm text-gray-600">
                    <div className="flex items-center gap-2">
                      ...
                      <span>{load.origin.name || `${load.origin.lat.toFixed(2)}, ${load.origin.lng.toFixed(2)}`} → {load.destination.name || `${load.destination.lat.toFixed(2)}, ${load.destination.lng.toFixed(2)}`}</span>
                    </div>
          ...
          {matchedLoads.slice(0, 5).map(load => (
            <div key={load.load_id} className="bg-green-50 border border-green-200 rounded-lg p-4">
              ...
              <div className="text-sm text-gray-600">
                ...
                <div className="mt-1">
                  {load.origin.name || `${load.origin.lat.toFixed(2)}, ${load.origin.lng.toFixed(2)}`} → {load.destination.name || `${load.destination.lat.toFixed(2)}, ${load.destination.lng.toFixed(2)}`}
                </div>
              </div>
            </div>
          ))}
